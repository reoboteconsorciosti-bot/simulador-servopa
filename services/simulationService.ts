...
export const calculateSimulation = (inputs: SimulationInputs): SimulationOutputs | null => {
  try {
    ...
    const N27_flag = seguroPrestamista === 1 ? 1 : 0; // Vida
    const N28_flag = seguroPrestamista === 2 ? 1 : 0; // Vida + Garantia

    const L14_seguro_vida = (L16_CONST * N12) * N27_flag;
    const L15_seguro_garantia = (L17_CONST * N12) * (N27_flag + N28_flag); // L17 aplica a ambos os seguros
